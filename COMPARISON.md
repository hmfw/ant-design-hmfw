...
### ✅ 已完成（53 个，全量 1297 测试通过，修复 149 个真实 bug）
...
Tooltip(修4bug) · Popover(修5bug) · Popconfirm(修8bug) · Modal(修9bug)

### ⏭️ 待办路线图（简单→复杂，按序进行）

- **D 浮层/反馈(剩余)**：drawer、message、notification、tour、image → 写入 [`COMPARISON/05-overlay-feedback.md`](./COMPARISON/05-overlay-feedback.md)
- **E 数据/复杂**：table、tree、transfer、carousel、color-picker → 写入 [`COMPARISON/06-data-complex.md`](./COMPARISON/06-data-complex.md)

**下一个执行**：D 浮层/反馈类（drawer 起，建议先 `/clear`）。恢复时对我说「继续」即可。
...
| 5 | D 浮层/反馈 | [05-overlay-feedback.md](./COMPARISON/05-overlay-feedback.md) | 🚧 4/9（已完成 Tooltip · Popover · Popconfirm · Modal） |
