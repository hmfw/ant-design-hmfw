...
test.describe('Popconfirm', () => {
  ...
  test('点击触发按钮弹出确认框', async ({ page }) => {
    const trigger = page.getByText(/^删\s*除$/).first()
    await trigger.click()
  ...
  test('确定和取消按钮存在', async ({ page }) => {
    ...
    const popconfirm = page.locator('.hmfw-popconfirm-inner').first()
    const okBtn = popconfirm.getByText(/^确\s*定$/).first()
    const cancelBtn = popconfirm.getByText(/^取\s*消$/).first()
