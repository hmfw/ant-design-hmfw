...
/**
 * 创建独立的 message 实例，支持配置传递
 * @param initialConfig 初始配置（duration/top/maxCount 等）
 * @returns MessageApi 实例
 */
export function createMessage(initialConfig?: ConfigOptions): MessageApi {
  ...
  let keyIndex = 0
  let instanceConfig: Required<Pick<ConfigOptions, 'duration' | 'top' | 'pauseOnHover'>> & ConfigOptions = {
  ...
  function startTimer(item: NoticeItem) {
    const duration = item.duration ?? instanceConfig.duration
    // duration <= 0 时不自动关闭，对齐 AntD
  ...
  function topStyle(): string {
    const { top } = instanceConfig
    return typeof top === 'number' ? `${top}px` : String(top ?? DEFAULT_TOP)
  }

  function ensureContainer() {
    const target = instanceConfig.getContainer?.() ?? document.body
              ...
              {notices.value.map((item) => {
                ...
                return (
                  <div
                    ...
                    onMouseenter={() => {
                      if (item.pauseOnHover ?? instanceConfig.pauseOnHover) clearTimer(item.mergedKey)
                    }}
                    onMouseleave={() => {
                      if (item.pauseOnHover ?? instanceConfig.pauseOnHover) startTimer(item)
                    }}
                  >
                    <div
                      class={cls(
                    ...
                    >
                      <div class="hmfw-message-notice-content">
                        {iconNode != null && (
                          <span
                            class={cls('hmfw-message-notice-icon', item.classNames?.icon)}
