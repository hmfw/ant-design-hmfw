...
const NotificationContainer = defineComponent({
  ...
  setup(props) {
    ...
    const add = (config: ArgsProps) => {
      ...
      const duration = config.duration ?? props.instanceConfig.duration ?? 4.5
      ...
      // Check maxCount
      const maxCount = props.instanceConfig.maxCount
    ...
    const handleMouseEnter = (item: NoticeItem) => {
      const pauseOnHover = item.pauseOnHover ?? props.instanceConfig.pauseOnHover ?? true
      if (pauseOnHover) {
        clearTimer(item.id)
        item.paused = true
      }
    ...
    const topStyle = computed(() => {
      ...
      const top = props.instanceConfig.top ?? 24
      const bottom = props.instanceConfig.bottom ?? 24
    ...
    const isRtl = computed(() => props.instanceConfig.rtl ?? false)
    ...
    return (
      <div class={cls('hmfw-notification', this.isRtl ? 'hmfw-notification-rtl' : '')} style={this.positionStyle}>
        {this.notices.map((notice) => {
          ...
          const showProgress =
            (notice.showProgress ?? this.$props.instanceConfig.showProgress ?? false) && notice.resolvedDuration > 0
...
/**
 * 创建独立的 notification 实例，支持配置传递
 * @param initialConfig 初始配置（duration/placement/maxCount 等）
 * @returns notification API 实例
 */
export function createNotification(initialConfig?: ConfigOptions) {
  ...
  // 使用 reactive 使容器内 computed（定位、rtl）能在 config() 更新后自动重算
  const instanceConfig: ConfigOptions = reactive({
    top: 24,
    bottom: 24,
    duration: 4.5,
    pauseOnHover: true,
  ...
  function getContainer(placement: NotificationPlacement) {
    ...
    const mountNode = instanceConfig.getContainer?.() ?? document.body
    const container = document.createElement('div')
    mountNode.appendChild(container)

    const app = createApp(NotificationContainer, { placement, instanceConfig })
  ...
  function open(config: ArgsProps) {
    const placement = config.placement ?? instanceConfig.placement ?? 'topRight'
  ...
  function config(options: ConfigOptions) {
    // instanceConfig 为 reactive，赋值后容器内依赖它的 computed（定位、rtl）会自动重算
    Object.assign(instanceConfig, options)
  }
