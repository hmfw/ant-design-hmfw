...
test.describe('Drawer', () => {
  ...
  test('打开/关闭 Drawer', async ({ page }) => {
    ...
    const drawer = page.locator('.hmfw-drawer-root')
  ...
  test('Escape 键关闭 Drawer', async ({ page }) => {
    await page.getByText('打开抽屉').first().click()
    await expect(page.locator('.hmfw-drawer-root')).toBeVisible()
    await page.keyboard.press('Escape')
    await expect(page.locator('.hmfw-drawer-root')).not.toBeVisible({ timeout: 3000 })
  })
