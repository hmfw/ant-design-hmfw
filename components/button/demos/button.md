...
通过设置 `size` 为 `large` `small` 分别把按钮设为大、小尺寸。若不设置 `size`，则尺寸默认为 `middle`。
...
借助 `classNames.root` 给按钮根节点叠加自定义样式层，即可实现渐变背景等复杂视觉效果，hover 时渐隐还原为主题色。

<DemoBlock title="渐变按钮" :source="ButtonGradientSource">
  <ButtonGradient />
</DemoBlock>
