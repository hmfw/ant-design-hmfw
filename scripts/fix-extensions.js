 ...
 *
 * 声明文件（.d.ts/.d.cts）同样补全：TS 约定在声明里写 `.js`/`.cjs` 后缀，
 * 解析时自动匹配同名 .d.ts/.d.cts。存在性检查以实际声明文件为准。
 */
...
// 收集 dist 下所有 .js / .cjs / .d.ts / .d.cts（跳过 sourcemap 与 UMD）
function collect(dir, out = []) {
  for (const e of readdirSync(dir, { withFileTypes: true })) {
    const full = join(dir, e.name)
    if (e.isDirectory()) collect(full, out)
    else if (/\.(js|cjs|d\.ts|d\.cts)$/.test(e.name) && !e.name.includes('.umd.')) out.push(full)
  }
  return out
}

// 把单个相对说明符解析为带扩展名的目标。
// importExt：写进代码里的后缀（.js/.cjs）；probeExt：用于存在性探测的实际文件后缀
// （声明文件探测 .d.ts/.d.cts，但写入仍用 .js/.cjs 让 TS 自动匹配）。
...
function extsFor(file) {
  if (file.endsWith('.d.cts')) return { importExt: '.cjs', probeExt: '.d.cts' }
  if (file.endsWith('.d.ts')) return { importExt: '.js', probeExt: '.d.ts' }
  if (file.endsWith('.cjs')) return { importExt: '.cjs', probeExt: '.cjs' }
