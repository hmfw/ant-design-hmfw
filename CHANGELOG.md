...
**✨ 新增功能**：新增 **Splitter** 分割面板组件，用于将页面或容器自由切分为多个可调整大小的区域。支持水平/垂直方向、受控与非受控尺寸（px 与百分比）、min/max 拖拽约束、折叠面板（含折叠图标三态与折叠动画）、懒加载拖拽预览、自定义拖拽/折叠图标、双击重置、完整拖拽事件体系（onResizeStart/onResize/onResizeEnd/onCollapse）、语义化 API（classNames/styles）、无障碍支持（role="separator" + aria-valuenow/min/max）。组件总数达到 **69 个**。全体 2594 单元测试通过。
...
**✨ 新增功能**：新增 **Masonry** 瀑布流布局组件，用于展示不同高度的内容。支持响应式列数（7 个断点：xxxl/xxl/xl/lg/md/sm/xs）、动态间距配置、RTL 布局、fresh 模式持续监听尺寸变化、布局变化回调、预指定列、语义化 API（classNames/styles）。瀑布流布局通过贪心算法将每个项目放置在当前最短的列中，实现视觉上的平衡和美观。组件总数达到 **68 个**。全体 2667 单元测试通过。

## [0.47.0] - 2026-08-17

[查看完整内容](./changelogs/v0.47.0.md)

**✨ 新增功能**：新增 **Listy** 虚拟列表组件，适用于大数据量列表场景的高性能渲染。支持虚拟滚动、仅渲染可视区域内的项目，可处理数万条数据流畅滚动。组件总数达到 **68 个**。全体 2656 单元测试通过。
...
ColorPicker 全面审查修复。修复**面板组件级 Token 在 Teleport 后无法解析**导致取色区高度塌陷为 0 的显示缺陷（按消费位置分两处定义 Token）、面板容器类名重复、灰色区拖拽色相跳变、拖拽全局监听器堆积、色相滑块缺少触摸支持、清除后状态残留。样式全面 Token 化，补齐清除按钮与禁用态无障碍语义，props 改用 `satisfies` 约束并导出 3 个事件处理器类型。`format` 文档如实标注当前仅实现 `hex`。新增 3 个 Demo（尺寸 / 禁用 / 清除），单元测试 12→17、E2E 5→9（含 Token 作用域回归测试），全体 2494 单元测试通过。
...
Tour 定位重写：修复滚动后高亮框错位、组件实例作为 `target` 崩溃、封面图 404、箭头无渲染，新增 `disabledInteraction`、补齐 dialog 无障碍语义、遮罩接入暗色 Token，单文件拆分为四模块。同时修复 Button 外部透传 `class` / `style` 覆盖组件自身样式的缺陷（影响所有传 class 的调用方）。`gap` 默认值对齐 AntD，属破坏性变更。
...
- **Radio**：补齐 `direction`、RadioButton 独立实现、语义化 API 与类型约束，事件对齐 AntD v6
- **RangePicker**：强化类型约束、抽取共享日期工具、修复面板标题国际化
- **Rate**：强化类型约束、补充事件类型导出与文案展现 Demo

### 📊 质量提升

- 覆盖 3 个组件，全部采用 `satisfies Record<keyof XProps, any>` 模式确保类型安全
- 日期处理逻辑抽取为共享工具模块，减少重复
