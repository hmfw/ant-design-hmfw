...
import { Tooltip, Button } from '@hmfw/ant-design'
<\/script>
`,G={class:"demo-dynamic-content"},K={style:{display:"flex",gap:"20px","margin-top":"20px"}},J=g({__name:"TooltipDynamicContent",setup(m){const i=v(!1),t=v(!1),b=T(()=>t.value?"这是一段很长的提示文字，用于演示当内容动态变化时，浮层位置会自动重新计算，确保不会溢出视口边界，提供更好的用户体验。":"短提示"),e=()=>{t.value=!t.value};return(r,f)=>(c(),u("div",G,[f[1]||(f[1]=n("p",null,"当浮层内容动态变化时，Tooltip 会通过 ResizeObserver 自动重新计算位置，确保浮层不会溢出视口。",-1)),n("div",K,[o(s(d),{title:b.value,placement:"top",open:i.value},{default:a(()=>[o(s(p),{onClick:f[0]||(f[0]=at=>i.value=!i.value)},{default:a(()=>[l(x(i.value?"关闭":"打开")+"提示",1)]),_:1})]),_:1},8,["title","open"]),o(s(p),{onClick:e},{default:a(()=>[l(x(t.value?"切换为短内容":"切换为长内容"),1)]),_:1})]),f[2]||(f[2]=n("p",{style:{"margin-top":"20px",color:"#666","font-size":"12px"}},"提示：打开浮层后切换内容长度，浮层位置会自动调整",-1))]))}}),Q=y(J,[["__scopeId","data-v-1da4fabc"]]),U=`<template>
