import{k as B,I as H,z as m,L as J,j as k,c as x,w as N,e as M,G as g,g as P,F as Q,d as y,E as R,B as W,M as E,h as X}from"./index-BNHhPN23.js";import{c as D}from"./cls-S9IiI6wd.js";const L=B({name:"Tree",props:{treeData:{type:Array,default:()=>[]},expandedKeys:Array,defaultExpandedKeys:{type:Array,default:()=>[]},defaultExpandAll:Boolean,selectedKeys:Array,defaultSelectedKeys:{type:Array,default:()=>[]},checkedKeys:Array,defaultCheckedKeys:{type:Array,default:()=>[]},checkable:Boolean,checkStrictly:Boolean,multiple:Boolean,selectable:{type:Boolean,default:!0},disabled:Boolean,showLine:Boolean,showIcon:Boolean,blockNode:Boolean,indent:{type:Number,default:24},fieldNames:Object},emits:["update:expandedKeys","update:selectedKeys","update:checkedKeys","expand","select","check"],setup(t,{emit:c}){const d=H("tree"),b=x(()=>{var e;return((e=t.fieldNames)==null?void 0:e.key)??"key"}),l=x(()=>{var e;return((e=t.fieldNames)==null?void 0:e.title)??"title"}),r=x(()=>{var e;return((e=t.fieldNames)==null?void 0:e.children)??"children"}),i=e=>e[b.value],U=e=>e[l.value],v=e=>e[r.value],S=e=>e.flatMap(a=>[i(a),...S(v(a)??[])]),C=m(t.defaultExpandAll?S(t.treeData):[...t.defaultExpandedKeys]),_=m([...t.defaultSelectedKeys]),j=m([...t.defaultCheckedKeys]),K=x(()=>t.expandedKeys??C.value),$=x(()=>t.selectedKeys??_.value),p=x(()=>t.checkedKeys??j.value);J(()=>t.treeData,()=>{t.defaultExpandAll&&(C.value=S(t.treeData))});const F=(e,a=0,n=null,s="0")=>e.flatMap((u,h)=>{const f=i(u),o=v(u),w=`${s}-${h}`,T=[{node:u,level:a,parentKey:n,hasChildren:!!(o!=null&&o.length),pos:w}];return o!=null&&o.length&&K.value.includes(f)&&T.push(...F(o,a+1,f,w)),T}),V=x(()=>F(t.treeData)),A=e=>e.flatMap(a=>{const n=v(a);return[i(a),...A(n??[])]}),I=(e,a,n=[])=>{for(const s of a){const u=v(s);if(i(s)===e)return n;if(u!=null&&u.length){const h=I(e,u,[...n,i(s)]);if(h.length>n.length)return h}}return n},z=e=>{const a=K.value.includes(e)?K.value.filter(n=>n!==e):[...K.value,e];C.value=a,c("update:expandedKeys",a),c("expand",a,{expanded:!K.value.includes(e),node:e})},G=(e,a)=>{if(!t.selectable||a.disabled||t.disabled)return;let n;t.multiple?n=$.value.includes(e)?$.value.filter(s=>s!==e):[...$.value,e]:n=$.value[0]===e?[]:[e],_.value=n,c("update:selectedKeys",n),c("select",n,{selected:n.includes(e),node:a})},O=(e,a)=>{if(a.disableCheckbox||a.disabled||t.disabled)return;const n=p.value.includes(e);let s;if(t.checkStrictly)s=n?p.value.filter(u=>u!==e):[...p.value,e];else{const u=v(a),h=A(u??[]);if(n){s=p.value.filter(o=>o!==e&&!h.includes(o));const f=I(e,t.treeData);s=s.filter(o=>!f.includes(o))}else s=[...new Set([...p.value,e,...h])]}j.value=s,c("update:checkedKeys",s),c("check",s,{checked:!n,node:a})};return()=>k("div",{class:D(d,{[`${d}-show-line`]:t.showLine,[`${d}-block-node`]:t.blockNode,[`${d}-disabled`]:t.disabled}),role:"tree","aria-multiselectable":t.multiple||void 0},[V.value.map(({node:e,level:a,hasChildren:n})=>{const s=i(e),u=U(e),h=K.value.includes(s),f=$.value.includes(s),o=p.value.includes(s),w=v(e),T=!o&&!t.checkStrictly&&!!(w&&A(w).some(q=>p.value.includes(q)));return k("div",{key:s,class:D(`${d}-treenode`,{[`${d}-treenode-selected`]:f,[`${d}-treenode-disabled`]:e.disabled||t.disabled,[`${d}-treenode-leaf`]:!n}),role:"treeitem","aria-level":a+1,"aria-expanded":n?h:void 0,"aria-selected":t.selectable?f:void 0,"aria-checked":t.checkable?o:void 0,"aria-disabled":e.disabled||t.disabled||void 0,style:{paddingLeft:`${a*t.indent}px`}},[k("span",{class:D(`${d}-switcher`,{[`${d}-switcher_open`]:n&&h,[`${d}-switcher_close`]:n&&!h,[`${d}-switcher-noop`]:!n}),onClick:()=>n&&z(s)},[n?h?"▾":"▸":null]),t.checkable&&k("span",{class:D(`${d}-checkbox`,{[`${d}-checkbox-checked`]:o,[`${d}-checkbox-indeterminate`]:T,[`${d}-checkbox-disabled`]:e.disableCheckbox||e.disabled||t.disabled}),onClick:()=>O(s,e)},[k("span",{class:`${d}-checkbox-inner`},null)]),k("span",{class:D(`${d}-node-content-wrapper`,{[`${d}-node-content-wrapper-normal`]:!n,[`${d}-node-selected`]:f}),onClick:()=>G(s,e)},[k("span",{class:`${d}-title`},[u])])])})])}}),Y=B({__name:"TreeBasic",setup(t){const c=m(["0-0","0-0-0"]),d=m([]),b=[{title:"父节点 1",key:"0-0",children:[{title:"父节点 1-0",key:"0-0-0",children:[{title:"叶子节点",key:"0-0-0-0"},{title:"叶子节点",key:"0-0-0-1"}]},{title:"父节点 1-1",key:"0-0-1",children:[{title:"叶子节点",key:"0-0-1-0"}]}]}];return(l,r)=>(N(),M(g(L),{"tree-data":b,"expanded-keys":c.value,"onUpdate:expandedKeys":r[0]||(r[0]=i=>c.value=i),"selected-keys":d.value,"onUpdate:selectedKeys":r[1]||(r[1]=i=>d.value=i)},null,8,["expanded-keys","selected-keys"]))}}),Z=`<template>
  <Tree
    :tree-data="treeData"
...
<\/script>
`,de=B({__name:"TreeShowLine",setup(t){const c=[{title:"父节点",key:"0-0",children:[{title:"子节点 1",key:"0-0-0"},{title:"子节点 2",key:"0-0-1"},{title:"子节点 3",key:"0-0-2"}]}];return(d,b)=>(N(),M(g(L),{"show-line":"","tree-data":c,"default-expand-all":!0}))}}),ne=`<template>
  <Tree
    show-line
    :tree-data="treeData"
