...
<\/script>
`,Ft={class:"upload-chunked-demo"},_t={key:0,class:"upload-chunked-log"},V=1024*1024,Rt=_({__name:"UploadChunked",setup(t){const a=C([]),d=C([]),n=g=>(d.value=[],d.value.push(`选中文件 ${g.name}，大小 ${(g.size/1024).toFixed(1)} KB`),!0),l=async g=>{const{file:u,onSuccess:h,onError:k,onProgress:B}=g;try{const v=Math.ceil(u.size/V);d.value.push(`开始分片：共 ${v} 片`);for(let D=0;D<v;D++){const I=D*V,E=Math.min(I+V,u.size),q=u.slice(I,E);await i(u.name,D,v,q);const O=Math.round((D+1)/v*95);B({percent:O}),d.value.push(`第 ${D+1}/${v} 片上传完成（${q.size} 字节）`)}await p(u.name,v),B({percent:100}),d.value.push("合并完成"),h({url:URL.createObjectURL(u)})}catch(v){k(v),d.value.push(`上传失败：${v.message}`)}};function i(g,u,h,k){return new Promise(B=>{const v=new FormData;v.append("name",g),v.append("index",String(u)),v.append("total",String(h)),v.append("chunk",k),setTimeout(B,300)})}function p(g,u){return new Promise(h=>{setTimeout(()=>h(),400)})}return(g,u)=>(x(),P("div",Ft,[o(U(R),{"file-list":a.value,"onUpdate:fileList":u[0]||(u[0]=h=>a.value=h),"custom-request":l,"before-upload":n,multiple:""},{default:b(()=>[o(U(S),null,{default:b(()=>[...u[1]||(u[1]=[c("分片上传",-1)])]),_:1})]),_:1},8,["file-list"]),d.value.length?(x(),P("div",_t,[(x(!0),P(Q,null,mt(d.value,(h,k)=>(x(),P("div",{key:k},gt(h),1))),128))])):Y("",!0)]))}}),Bt=ht(Rt,[["__scopeId","data-v-eec3cc2f"]]),Pt=`<template>
...
`,St=_({__name:"UploadCustom",setup(t){const a=C([]),d=i=>i.type.startsWith("image/")?i.size/1024/1024<2?!0:(console.error("图片大小不能超过 2MB！"),!1):(console.error("只能上传图片文件！"),!1),n=({file:i,onSuccess:p,onProgress:g})=>{let u=0;const h=setInterval(()=>{u+=20,g({percent:u}),u>=100&&(clearInterval(h),p({url:URL.createObjectURL(i)}))},200)},l=i=>{a.value=i.fileList};return(i,p)=>(x(),z(U(R),{"file-list":a.value,"onUpdate:fileList":p[0]||(p[0]=g=>a.value=g),"custom-request":n,"before-upload":d,accept:".jpg,.jpeg,.png,.gif",onChange:l},{default:b(()=>[o(U(S),null,{default:b(()=>[...p[1]||(p[1]=[c("选择图片",-1)])]),_:1})]),_:1},8,["file-list"]))}}),It=`<template>
  <Upload
    v-model:file-list="fileList"
...
<\/script>
`,qt=_({__name:"UploadDragger",setup(t){const a=C([]),d=l=>{console.log("文件状态：",l.file.status),a.value=l.fileList},n=l=>{var i;console.log("拖拽文件：",(i=l.dataTransfer)==null?void 0:i.files)};return(l,i)=>(x(),z(U(tt),{"file-list":a.value,"onUpdate:fileList":i[0]||(i[0]=p=>a.value=p),action:"https://run.mocky.io/v3/435e224c-44fb-4773-9faf-380c5e6a2188",multiple:!0,onChange:d,onDrop:n},{default:b(()=>[...i[1]||(i[1]=[f("div",{style:{padding:"32px","text-align":"center"}},[f("p",{style:{"font-size":"48px","margin-bottom":"8px"}},"📥"),f("p",{style:{"font-size":"16px","margin-bottom":"4px"}},"点击或拖拽文件到此区域上传"),f("p",{style:{color:"#999","font-size":"14px"}},"支持单个或批量上传，严禁上传公司数据或其他违禁文件")],-1)])]),_:1},8,["file-list"]))}}),Tt=`<template>
  <UploadDragger
    v-model:file-list="fileList"
    action="https://run.mocky.io/v3/435e224c-44fb-4773-9faf-380c5e6a2188"
    :multiple="true"
...
`,Et={key:0},Ot=_({__name:"UploadPictureCard",setup(t){const a=C([{uid:"-1",name:"image.png",status:"done",url:"https://zos.alipayobjects.com/rmsportal/jkjgkEfvpUPVyRjUImniVslZfWPnJuuZ.png"}]),d=l=>{console.log("预览：",l.url||l.thumbUrl)},n=l=>{a.value=l.fileList};return(l,i)=>(x(),z(U(R),{"file-list":a.value,"onUpdate:fileList":i[0]||(i[0]=p=>a.value=p),action:"https://run.mocky.io/v3/435e224c-44fb-4773-9faf-380c5e6a2188","list-type":"picture-card","max-count":8,onPreview:d,onChange:n},{default:b(()=>[a.value.length<8?(x(),P("div",Et,[...i[1]||(i[1]=[f("div",null,"+ 上传",-1)])])):Y("",!0)]),_:1},8,["file-list"]))}}),Mt=`<template>
  <Upload
    v-model:file-list="fileList"
    action="https://run.mocky.io/v3/435e224c-44fb-4773-9faf-380c5e6a2188"
