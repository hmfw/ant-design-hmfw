...
<\/script>
`,Pe={style:{"margin-top":"12px",color:"rgba(0, 0, 0, 0.45)","font-size":"12px"}},Fe=B({__name:"TransferDraggable",setup(t){const r=Array.from({length:8},(n,l)=>({key:String(l),title:`Item ${l+1}`})),c=w(["0","1","2","3"]);function y(n){console.log("reorder",n)}return(n,l)=>(j(),ye(le,null,[o(h(M),{"target-keys":c.value,"onUpdate:targetKeys":l[0]||(l[0]=x=>c.value=x),"data-source":h(r),titles:["源","目标（可拖拽排序）"],draggable:"",onReorder:y},null,8,["target-keys","data-source"]),m("p",Pe," 当前顺序："+Ie(c.value.join(", ")||"（空）"),1)],64))}}),je=`<template>
  <Transfer
    v-model:target-keys="targetKeys"
    :data-source="dataSource"
    :titles="['源', '目标（可拖拽排序）']"
