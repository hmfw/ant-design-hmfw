...
<\/script>
`,Ft={class:"upload-chunked-demo"},_t={key:0,class:"upload-chunked-log"},V=1024*1024,Rt=_({__name:"UploadChunked",setup(t){const a=C([]),d=C([]),n=g=>(d.value=[],d.value.push(`选中文件 ${g.name}，大小 ${(g.size/1024).toFixed(1)} KB`),!0),l=async g=>{const{file:u,onSuccess:h,onError:k,onProgress:P}=g;try{const v=Math.ceil(u.size/V);d.value.push(`开始分片：共 ${v} 片`);for(let D=0;D<v;D++){const S=D*V,z=Math.min(S+V,u.size),I=u.slice(S,z);await i(u.name,D,v,I);const E=Math.round((D+1)/v*95);P({percent:E}),d.value.push(`第 ${D+1}/${v} 片上传完成（${I.size} 字节）`)}await p(u.name,v),P({percent:100}),d.value.push("合并完成"),h({url:URL.createObjectURL(u)})}catch(v){k(v),d.value.push(`上传失败：${v.message}`)}};function i(g,u,h,k){return new Promise(P=>{const v=new FormData;v.append("name",g),v.append("index",String(u)),v.append("total",String(h)),v.append("chunk",k),setTimeout(P,300)})}function p(g,u){return new Promise(h=>{setTimeout(()=>h(),400)})}return(g,u)=>(x(),B("div",Ft,[o(U(R),{"file-list":a.value,"onUpdate:fileList":u[0]||(u[0]=h=>a.value=h),"custom-request":l,"before-upload":n,multiple:""},{default:b(()=>[o(U(O),null,{default:b(()=>[...u[1]||(u[1]=[c("分片上传",-1)])]),_:1})]),_:1},8,["file-list"]),d.value.length?(x(),B("div",_t,[(x(!0),B(Q,null,mt(d.value,(h,k)=>(x(),B("div",{key:k},gt(h),1))),128))])):Y("",!0)]))}}),Pt=ht(Rt,[["__scopeId","data-v-eec3cc2f"]]),Bt=`<template>
...
`,St=_({__name:"UploadCustom",setup(t){const a=C([]),d=i=>i.type.startsWith("image/")?i.size/1024/1024<2?!0:(console.error("图片大小不能超过 2MB！"),!1):(console.error("只能上传图片文件！"),!1),n=({file:i,onSuccess:p,onProgress:g})=>{let u=0;const h=setInterval(()=>{u+=20,g({percent:u}),u>=100&&(clearInterval(h),p({url:URL.createObjectURL(i)}))},200)},l=i=>{a.value=i.fileList};return(i,p)=>(x(),j(U(R),{"file-list":a.value,"onUpdate:fileList":p[0]||(p[0]=g=>a.value=g),"custom-request":n,"before-upload":d,accept:".jpg,.jpeg,.png,.gif",onChange:l},{default:b(()=>[o(U(O),null,{default:b(()=>[...p[1]||(p[1]=[c("选择图片",-1)])]),_:1})]),_:1},8,["file-list"]))}}),It=`<template>
  <Upload
    v-model:file-list="fileList"
...
<\/script>
`,qt=_({__name:"UploadDragger",setup(t){const a=C([]),d=l=>{console.log("文件状态：",l.file.status),a.value=l.fileList},n=l=>{var i;console.log("拖拽文件：",(i=l.dataTransfer)==null?void 0:i.files)};return(l,i)=>(x(),j(U(tt),{"file-list":a.value,"onUpdate:fileList":i[0]||(i[0]=p=>a.value=p),action:"https://run.mocky.io/v3/435e224c-44fb-4773-9faf-380c5e6a2188",multiple:!0,onChange:d,onDrop:n},{default:b(()=>[...i[1]||(i[1]=[f("div",{style:{padding:"32px","text-align":"center"}},[f("p",{style:{"font-size":"48px","margin-bottom":"8px"}},"📥"),f("p",{style:{"font-size":"16px","margin-bottom":"4px"}},"点击或拖拽文件到此区域上传"),f("p",{style:{color:"#999","font-size":"14px"}},"支持单个或批量上传，严禁上传公司数据或其他违禁文件")],-1)])]),_:1},8,["file-list"]))}}),Tt=`<template>
  <UploadDragger
    v-model:file-list="fileList"
    action="https://run.mocky.io/v3/435e224c-44fb-4773-9faf-380c5e6a2188"
    :multiple="true"
...
<\/script>
`,jt=_({__name:"UploadItemRender",setup(t){const a=C([{uid:"-1",name:"design.pdf",status:"done",url:"https://example.com/design.pdf",size:204800},{uid:"-2",name:"cover.png",status:"done",url:"https://example.com/cover.png",size:102400}]),d=({onSuccess:l,onProgress:i})=>{let p=0;const g=setInterval(()=>{p+=25,i({percent:p}),p>=100&&(clearInterval(g),l({}))},200)},n=(l,i,p,g)=>o("div",{class:"custom-upload-item","data-status":i.status},[o("span",{class:"custom-upload-item-name"},[c("📎 "),i.name]),o("span",{class:"custom-upload-item-status"},[i.status]),o("button",{class:"custom-upload-item-btn",onClick:g.preview},[c("预览")]),o("button",{class:"custom-upload-item-btn danger",onClick:g.remove},[c("删除")])]);return(l,i)=>(x(),j(U(R),{"file-list":a.value,"onUpdate:fileList":i[0]||(i[0]=p=>a.value=p),"custom-request":d,"item-render":n,multiple:""},{default:b(()=>[o(U(O),null,{default:b(()=>[...i[1]||(i[1]=[c("自定义渲染",-1)])]),_:1})]),_:1},8,["file-list"]))}}),zt=`<template>
...
`,Et={key:0},Ot=_({__name:"UploadPictureCard",setup(t){const a=C([{uid:"-1",name:"image.png",status:"done",url:"https://zos.alipayobjects.com/rmsportal/jkjgkEfvpUPVyRjUImniVslZfWPnJuuZ.png"}]),d=l=>{console.log("预览：",l.url||l.thumbUrl)},n=l=>{a.value=l.fileList};return(l,i)=>(x(),j(U(R),{"file-list":a.value,"onUpdate:fileList":i[0]||(i[0]=p=>a.value=p),action:"https://run.mocky.io/v3/435e224c-44fb-4773-9faf-380c5e6a2188","list-type":"picture-card","max-count":8,onPreview:d,onChange:n},{default:b(()=>[a.value.length<8?(x(),B("div",Et,[...i[1]||(i[1]=[f("div",null,"+ 上传",-1)])])):Y("",!0)]),_:1},8,["file-list"]))}}),Mt=`<template>
  <Upload
    v-model:file-list="fileList"
    action="https://run.mocky.io/v3/435e224c-44fb-4773-9faf-380c5e6a2188"
