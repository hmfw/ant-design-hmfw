...
**执行此 skill 前，必须先向用户确认以下理解，得到明确同意后才能继续：**

### 执行计划概述

1. **5 个阶段**：代码审查 → Demo 覆盖分析 → 创建任务 → 逐项修复 → 生成总结
2. **4 份临时文档**：
   - `CODE_REVIEW.md` — 代码问题清单与修复建议
   - `DEMO_COVERAGE_ANALYSIS.md` — Demo 覆盖率分析与缺失清单
   - `FIXES_SUMMARY.md` — 代码修复总结
   - `DEMO_SUPPLEMENT_SUMMARY.md` — Demo 补充总结
   - ⚠️ **这 4 份文档是临时产物，验证通过后会被删除，不提交入库**
3. **代码改动保留**：Props 规范修复、魔数提取、新增 demo 等会保留并提交

### 必检项（P0 优先级）

根据项目 `CLAUDE.md` 规范，以下问题必须检查：

- ✅ **Props 类型约束**：是否使用 `satisfies Record<keyof XProps, any>` 模式
- ✅ **边界条件防御**：数值 props 是否有 `Math.max/min` 保护
- ✅ **VNode 过滤**：是否正确识别函数组件（`typeof vnode.type === 'function'`）
- ✅ **CSS Token 系统**：样式是否消费 `var(--hmfw-*)` 而非硬编码颜色/圆角/间距
- ✅ **算法注释**：复杂逻辑（>20 行）是否有中文注释

### Demo 必补项（P0 优先级）

- ✅ **基础用法** demo
- ✅ **Slot 语法** demo（如果组件支持 items + slot 双 API）
- ✅ **语义化 API** demo（如果组件实现了 classNames/styles）

### 预期耗时与产物

- ⏱️ **预计耗时**：1-3 小时（取决于问题数量）
- 📄 **保留产物**：代码修复、新增 demo、更新的文档
- 🗑️ **删除产物**：4 份临时报告文档（验证后）
...
```
我将对 {ComponentName} 组件执行 component-review skill：
- 生成 4 份临时报告（验证后删除）
- 检查 Props 规范、CSS Token、边界条件等必检项
- 按优先级修复发现的问题（P0 → P1 → P2 → P3）
- 补充缺失的 demo（基础用法、Slot、语义化 API）
- 预计耗时 1-3 小时

是否继续？
```

**得到用户明确同意后，才能进入下一步"审查流程"。**
